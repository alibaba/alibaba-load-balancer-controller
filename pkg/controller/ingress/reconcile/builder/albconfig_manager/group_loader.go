...
func NewDefaultGroupLoader(kubeClient client.Client, client apiextcli.Interface, annotationParser annotations.Parser) *defaultGroupLoader {
...
func (m *defaultGroupLoader) LoadGroupID(ctx context.Context, ing *networking.Ingress) (*GroupID, error) {
	...
	err = m.kubeClient.List(ctx, alist)
	if err == nil {
		hasAlbConfig := ""
		klog.Infof("alist %d", len(alist.Items))
		for _, item := range alist.Items {
			if item.Name == groupName {
				klog.Infof("alist groupName %s", groupName)
...
func (m *defaultGroupLoader) IngressClass(ing *networking.Ingress) string {
	...
	err := m.kubeClient.Get(context.TODO(), types.NamespacedName{Name: *alb}, ic)
	if err != nil {
