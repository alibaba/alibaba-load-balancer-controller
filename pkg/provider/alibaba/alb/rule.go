...
func transSDKTrafficLimitConfigToCreateRules(trafficLimitConf alb.TrafficLimitConfig) albsdk.CreateRulesRulesRuleActionsItemTrafficLimitConfig {
	return albsdk.CreateRulesRulesRuleActionsItemTrafficLimitConfig{
		QPS:      limitNumber2String(trafficLimitConf.QPS),
		PerIpQps: limitNumber2String(trafficLimitConf.PerIpQps),
	}
}
...
func limitNumber2String(limit int) string {
	if limit <= 0 {
		return ""
	}
...
func transModelActionToSDK(action alb.Action) (*albsdk.Action, error) {
	...
	case util.RuleActionTypeTrafficLimit:
		sdkObj.TrafficLimitConfig = albsdk.TrafficLimitConfig{
			...
			PerIpQps: action.TrafficLimitConfig.PerIpQps,
		}
	case util.RuleActionTypeTrafficMirror:
...
func (m *ALBProvider) BatchUpdateRules(ctx context.Context, ruleMgr *ALBProvider, rules []albsdk.UpdateRulesAttributeRules, cnt int, batch BatchUpdateRulesFunc, lsID string) error {
	if cnt <= 0 || cnt >= util.BatchCreateDeleteUpdateRulesMaxNum {
	...
	for total > cnt {
		...
		total = len(rules)
		if _, err := m.waitListenerStatus(ctx, lsID); err != nil {
			return err
		}
	}
	if total <= 0 {
...
func (m *ALBProvider) UpdateALBListenerRules(ctx context.Context, rulePairs []alb.ResAndSDKListenerRulePair) error {
	...
	lsID, err := (*rulePairs[0].ResLR).Spec.ListenerID.Resolve(ctx)
	if err != nil {
		return err
	}
	if err := m.BatchUpdateRules(ctx, m, updateRules, util.BatchUpdateRulesDefaultNum, updateRulesFunc, lsID); err != nil {
		return err
	}
	return nil
}
